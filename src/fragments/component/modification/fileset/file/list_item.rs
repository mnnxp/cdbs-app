    ...
    fn update(&mut self, ctx: &Context<Self>, msg: Self::Message) -> bool {
        ...
        match msg {
            Msg::RequestDownloadFile => {
                let select_fileset_uuid = ctx.props().select_fileset_uuid.clone();
            ...
            Msg::GetDownloadFileResult(res) => {
                let result_parsing: Vec::<DownloadFile> = resp_parsing(res, "componentModificationFilesetFiles")
                    .map_err(|err| link.send_message(Msg::ResponseError(err)))
                    .unwrap();
                self.download_url = result_parsing.first().map(|f| f.download_url.clone()).unwrap_or_default();
            },
            Msg::GetDeleteFileResult(res) => {
                self.get_result_delete = resp_parsing_item(res, "deleteFilesFromFileset")
                    .map_err(|err| link.send_message(Msg::ResponseError(err)))
                    .unwrap();
