...
use crate::gqls::supplier_service::{DeleteServiceFile, delete_service_file};
        ...
        match msg {
            Msg::RequestDeleteFile(file_uuid) => {
                ...
                spawn_local(async move {
                    let delete_service_file_data = delete_service_file::DeleteServiceFileData{
                        fileUuid: file_uuid.clone(),
                        serviceUuid: service_uuid,
                    };
                    let res = make_query(DeleteServiceFile::build_query(delete_service_file::Variables {
                        delete_service_file_data
                    })).await.unwrap();
            ...
            Msg::GetDeleteFileResult(res, file_uuid) => {
                match resp_parsing(res, "deleteServiceFile") {
                    Ok(result) => {
