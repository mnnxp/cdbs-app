...
use crate::gqls::{
    ...
    supplier_service::{
        ...
        GetServiceParams, get_service_params,
    },
};
...
pub struct ServiceParamsTags {
    error: Option<Error>,
    props: Props,
    link: ComponentLink<Self>,
    param_ids: BTreeSet<usize>,
    service_params: Vec<ServiceParam>,
...
impl Component for ServiceParamsTags {
    type Message = Msg;
    ...
    fn rendered(&mut self, first_render: bool) {
        if first_render {
            ...
            self.link.send_message(Msg::RequestServiceParams);
        }
    }
    ...
    fn update(&mut self, msg: Self::Message) -> ShouldRender {
        ...
        match msg {
            Msg::DeleteServiceParam(param_id) => {
            ...
            Msg::RequestServiceParams => {
                if self.props.service_uuid.len() != 36 {
                    return true
                }
                let service_uuid = self.props.service_uuid.clone();
                let ipt_paginate = Some(get_service_params::IptPaginate {
                    currentPage: self.page_set.current_page,
                    perPage: self.page_set.per_page,
                });
                spawn_local(async move {
                    let res = make_query(GetServiceParams::build_query(
                        get_service_params::Variables { service_uuid, ipt_sort: None, ipt_paginate }
                    )).await.unwrap();
                    link.send_message(Msg::GetServiceParamsResult(res));
                })
            },
            ...
            Msg::GetAddParamResult(res) => {
                match resp_parsing::<usize>(res, "putServiceParams") {
                    Ok(result) => {
                        ...
                        link.send_message(Msg::RequestServiceParams);
                    },
                    Err(err) => link.send_message(Msg::ResponseError(err)),
                }
            },
            Msg::GetServiceParamsResult(res) => {
                match resp_parsing_two_level(res, "service", "serviceParams") {
                    Ok(result) => {
                        debug!("serviceParams: {:?}", result);
                        self.service_params = result;
                        self.param_ids = BTreeSet::new();
                        for param in &self.service_params {
...
impl ServiceParamsTags {
    fn show_params(&self) -> Html {
        let onclick_delete_param = match self.props.show_manage_btn {
            true => Some(self.link.callback(|value: usize| Msg::DeleteServiceParam(value))),
            false => None,
        };
        let classes_table = get_classes_table(self.service_params.len());
        ...
        html!{
            ...
            <div class={"table-container"}>
                <table class={classes_table}>
                    ...
                    <tbody>
                        {for self.service_params.iter().enumerate().map(|(numer, data)| {
