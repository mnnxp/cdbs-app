...
use crate::gqls::supplier_service::{DeleteServiceKeywords, delete_service_keywords};
        ...
        match msg {
            Msg::RequestDeleteKeyword => {
                ...
                spawn_local(async move {
                    let ipt_service_keywords_data = delete_service_keywords::IptServiceKeywordsData{
                        serviceUuid: service_uuid,
                        keywordIds: vec![keyword_id],
                    };
                    let res = make_query(DeleteServiceKeywords::build_query(delete_service_keywords::Variables {
                        ipt_service_keywords_data,
                    })).await.unwrap();
            ...
            Msg::GetDeleteKeywordResult(res) => {
                match resp_parsing::<usize>(res, "deleteServiceKeywords") {
                    Ok(result) => {
                        debug!("deleteServiceKeywords: {:?}", result);
